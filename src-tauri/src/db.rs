    ...
    match existing {
        Some(saved_key) if saved_key == trimmed => {
            if let Err(error) = prepare_remote_sync(&conn, &server_url, trimmed, &device_id) {
                let snapshot =
                    load_snapshot(&conn).map_err(|snapshot_error| snapshot_error.to_string())?;
                return Ok(UnlockResponse {
                    unlocked: true,
                    sync_key: Some(saved_key),
                    message: format!("서버에 연결하지 못해 오프라인 모드로 엽니다. {}", error),
                    snapshot: Some(snapshot),
                });
            }

            let snapshot = match sync_with_server(&conn, &server_url, trimmed, &device_id) {
                Ok(execution) => execution.snapshot,
                Err(error) => {
                    let snapshot = load_snapshot(&conn)
                        .map_err(|snapshot_error| snapshot_error.to_string())?;
                    return Ok(UnlockResponse {
                        unlocked: true,
                        sync_key: Some(saved_key),
                        message: format!("동기화 없이 오프라인 모드로 엽니다. {}", error),
                        snapshot: Some(snapshot),
                    });
                }
            };
...
pub fn regenerate_sync_key(state: State<AppState>) -> Result<SyncKeyResponse, String> {
    ...
    let sync_key = match get_setting(&conn, "sync_key").map_err(|error| error.to_string())? {
        Some(existing_key) => {
            ...
            let next_key = regenerate_remote_key(&client, &server_url, &existing_key, &device_id)
...
pub fn sync_now(state: State<AppState>) -> Result<SyncActionResponse, String> {
    ...
    let server_url = get_sync_server_url(&conn).map_err(|error| error.to_string())?;
    let device_id = get_or_create_device_id(&conn).map_err(|error| error.to_string())?;
    prepare_remote_sync(&conn, &server_url, &sync_key, &device_id)
        .map_err(|error| error.to_string())?;
    let execution = sync_with_server(&conn, &server_url, &sync_key, &device_id)
        .map_err(|error| error.to_string())?;
...
pub fn create_routine(input: RoutineInput, state: State<AppState>) -> Result<AppSnapshot, String> {
    ...
    with_write_transaction(&conn, |conn| {
        ...
        queue_outbox(conn, "routine", &id, "create", &payload)
            .map_err(|error| error.to_string())?;
        load_snapshot(conn).map_err(|error| error.to_string())
    })
}
...
pub fn update_routine(
    ...
    with_write_transaction(&conn, |conn| {
        ...
        queue_outbox(conn, "routine", &input.id, "update", &payload)
            .map_err(|error| error.to_string())?;
        load_snapshot(conn).map_err(|error| error.to_string())
    })
}
...
fn sync_with_server(
    ...
    let remote_apply_stats = apply_remote_events(conn, &response.changes)?;
    ...
    Ok(SyncExecution {
        ...
        pushed_count: response.acked_event_ids.len() as i64,
        pulled_count: remote_apply_stats.pulled_count,
        conflict_count: remote_apply_stats.conflict_count,
    })
}

fn prepare_remote_sync(
    conn: &Connection,
    server_url: &str,
    sync_key: &str,
    device_id: &str,
...
fn load_local_routine_for_sync(
    conn: &Connection,
    routine_id: &str,
...
fn get_local_routine_updated_at(conn: &Connection, routine_id: &str) -> Result<Option<i64>> {
    conn.query_row(
        "SELECT updated_at FROM routines WHERE id = ?1",
        [routine_id],
        |row| row.get::<_, i64>(0),
    )
    .optional()
    .context("루틴 수정 시각을 읽지 못했습니다.")
}
...
fn apply_remote_events(conn: &Connection, events: &[RemoteSyncEvent]) -> Result<RemoteApplyStats> {
    let mut pulled_count = 0_i64;
            ...
            "routine" => {
             ...
             WHERE id = ?1
               AND COALESCE(updated_at, 0) <= ?2",
                    ...
                    if affected > 0 {
                        pulled_count += 1;
                    } else if local_updated_at.is_some_and(|local| local > updated_at) {
                        conflict_count += 1;
                    }
                    continue;
                }
...
fn generate_sync_key() -> String {
    ...
    format!("DC-{}-{}-{}", &token[0..4], &token[4..8], &token[8..12])
}
...
fn timestamp_to_display(value: &str) -> String {
    let timestamp = value
        .parse::<i64>()
        .unwrap_or_else(|_| iso_to_timestamp(value));
...
mod tests {
    ...
    fn offline_crud_updates_local_snapshot_and_outbox() {
        ...
        insert_routine_event(&conn, routine_id, "오프라인 생성", 1_710_000_001)
            .expect("create routine");
        update_routine_title_event(&conn, routine_id, "오프라인 수정", 1_710_000_002)
            .expect("update routine");
        toggle_check_event(&conn, routine_id, "2026-03-30", true, 1_710_000_003)
            .expect("toggle on");
        ...
        assert_eq!(
            snapshot.routines[0].completed_dates,
    ...
    fn online_sync_replays_crud_between_two_devices() {
        ...
        insert_routine_event(&conn_a, "shared-routine", "기기 간 루틴", 1_710_000_101)
            .expect("device a create");
        ...
        update_routine_title_event(&conn_b, "shared-routine", "기기 B 수정", 1_710_000_102)
            .expect("device b update");
        let snapshot_b_synced =
            sync_existing_device(&conn_b, &sync_key).expect("device b sync update");
        assert_eq!(snapshot_b_synced.routines[0].title, "기기 B 수정");

        let snapshot_a_after_update =
            sync_existing_device(&conn_a, &sync_key).expect("device a pull update");
        assert_eq!(snapshot_a_after_update.routines[0].title, "기기 B 수정");

        toggle_check_event(&conn_a, "shared-routine", "2026-03-31", true, 1_710_000_103)
            .expect("device a check");
        sync_existing_device(&conn_a, &sync_key).expect("device a sync check");
        let snapshot_b_after_check =
            sync_existing_device(&conn_b, &sync_key).expect("device b pull check");
        assert_eq!(
            snapshot_b_after_check.routines[0].completed_dates,
        ...
        let snapshot_a_after_delete =
            sync_existing_device(&conn_a, &sync_key).expect("device a pull delete");
    ...
    fn stale_remote_event_does_not_override_newer_state() {
        ...
        insert_routine_event(&conn_a, "conflict-routine", "초기 루틴", 1_710_000_201)
            .expect("seed conflict routine");
        connect_new_device(&conn_a, &server_url, &sync_key).expect("device a initial sync");
        connect_new_device(&conn_b, &server_url, &sync_key).expect("device b initial pull");

        update_routine_title_event(&conn_b, "conflict-routine", "더 최신 값", 1_710_000_300)
            .expect("device b newer update");
        sync_existing_device(&conn_b, &sync_key).expect("device b sync newer update");

        update_routine_title_event(&conn_a, "conflict-routine", "오래된 값", 1_710_000_250)
            .expect("device a stale update");
        let snapshot_a =
            sync_existing_device(&conn_a, &sync_key).expect("device a sync stale update");
        assert_eq!(
            find_routine_title(&snapshot_a, "conflict-routine"),
            Some("더 최신 값")
        );
    ...
    fn connect_new_device(
        ...
        sync_with_server(conn, server_url, sync_key, &device_id).map(|execution| execution.snapshot)
    }
