...
1. 개발용이면 `sync-server/.env.development.example`, 배포용이면 `sync-server/.env.production.example`을 복사해 환경파일을 만듭니다.
...
cp sync-server/.env.development.example sync-server/.env.development
...
```bash
cp sync-server/.env.production.example sync-server/.env.production
